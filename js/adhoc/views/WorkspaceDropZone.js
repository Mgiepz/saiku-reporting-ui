...
var WorkspaceDropZone = Backbone.View.extend({
    ...
    select_dimension: function(event, ui) {
        ...
        var $original = $(this.workspace.el).find('.sidebar').find('a[href="' + original_href + '"]').parent('li');
        $original.css({
            fontWeight: "bold"
        })
        //.draggable('disable')
        ;
        $original.parents('.parent_dimension').find('.root').css({
            fontWeight: "bold"
        });
        ...
        this.workspace.query.move_dimension(dimension, $(event.target).parent(), undefined, index);
    ...
    move_dimension: function(event, ui) {
        ...
        if(!ui.item.hasClass('deleted')) {
            ...
            //$(event.target).parent()
            ui.item.parents('.fields_list_body'), indexFrom, index);
        }
    ...
    remove_dimension: function(event, ui) {
        ...
        var $original = $(this.workspace.el).find('.sidebar').find('a[href="' + original_href + '"]').parent('li');
        $original.draggable('enable').css({
            fontWeight: 'normal'
        });

        // Unhighlight the parent if applicable
        if($original.parents('.parent_dimension').children().children('.ui-state-disabled').length === 0) {
            $original.parents('.parent_dimension').find('.root').css({
                fontWeight: "normal"
            });
        }
    ...
    selections: function(event, ui) {
        ...
        if($target.parents('.fields_list').attr('title') == 'MEASURES') {
            ...
            //we allready need to decide here if it is a calculated column
